  ...
  return (
    <section className="space-y-6" aria-labelledby="projects-heading">
      <header className="flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 pb-2 transition-colors duration-300">
        ...
        <h2 id="projects-heading" className="text-2xl font-bold text-gray-900 dark:text-gray-100 transition-colors duration-300">{title}</h2>
      </header>
        ...
        {projects.map((project, idx) => (
          <Card key={idx} className="hover:shadow-md transition-shadow flex flex-col h-full">
            <header className="flex justify-between items-start mb-2">
              ...
              {project.period && (
                <time className="text-xs font-mono font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded-md transition-colors duration-300">
                  {project.period}
                </time>
              )}
            </header>
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-500 dark:text-gray-400 transition-colors duration-300">{project.description}</p>
              {project.link && (
                <a 
                  href={project.link} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="inline-flex items-center gap-1 mt-2 text-sm font-medium text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 transition-colors focus:ring-2 focus:ring-blue-600 dark:focus:ring-blue-500 rounded-md outline-none"
                >
